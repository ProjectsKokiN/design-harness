...
iOS 102行 × Android 70行の `none` から、両方で成り立つと確かめられた規約は
**25件**です（`build/shared-rules.md`）。**片方が分類を直すたびに増えます**
（2026-09-06、Windows が6件を直して2件増えました）。**減ることもあります**——2026-09-07 に規約15 を取り下げました（片側の根拠しか無かった）。取り下げた分は `1` 件で、`build/shared-rules.md` の末尾に**消さずに**残しています。
