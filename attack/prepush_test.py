...
実害（FlashEnglish・2026-09-04）: マージ済みの枝6本を消そうとして
...
CASES = [
    ...
    ("ふつうの push は検査に入る",
     f"refs/heads/work {SHA} refs/heads/work {ZERO}\n",
     0, "検査を回しました", None),
    # main 直 push の禁止は 2026-09-05 に廃止（ユーザー確定）。**止めないことを試験する。**
    # 廃止したのに雛形に分岐が残っていた実例があるので、逆向きに固定しておく
    ("Mac から main へ直 push できる（2026-09-05 に禁止を廃止）",
     f"refs/heads/main {SHA} refs/heads/main {ZERO}\n",
     0, "検査を回しました", "直 push は止めています"),
    ("main 宛ての削除は検査を飛ばす（送る中身が無い）",
     f"(delete) {ZERO} refs/heads/main {SHA}\n",
     0, "検査を飛ばします", "直 push は止めています"),
    ("Mac 以外でも main へ push できる",
     f"refs/heads/main {SHA} refs/heads/main {ZERO}\n",
     0, "検査を回しました", "直 push は止めています"),
