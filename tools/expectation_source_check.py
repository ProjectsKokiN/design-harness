...
- **期待値と実測が同じ実装から来ている**こと（#70・aub 2026-09-04: `cropFraction()` と
  `square()` が同じ関数を呼んでいて、壊しても両辺で打ち消し合って通った）。ここは
  「書き出し由来か」だけを見る。同じ出どころどうしの突き合わせは `hollow_check.py` の
  形8（打ち消し合い）が見る。**期待値の出どころを決めるときの問い**は2つ:
  (1) 書き出しから来ているか、(2) **実装と同じ関数・同じファイルから来ていないか**。
  (2) に当たるなら、描いた値（`getSize` / `getRect`）か別の書き出しと1つは突き合わせる
