...
いたため、共通側を直しても案件へ届きませんでした（FlashEnglish は156行・10.5KB の
...
3. 案件側の見出しが required-sections だけか（**許可リスト方式**。写しの再発防止で、
   ...
   FlashEnglish の「原則（5つ）」は、共通側の「設計原則」と文言が違うせいで
   すり抜けていました
...
#: FlashEnglish は「原則（5つ）」「禁止パターン（概要）」「主要トークン（抜粋）」を
...
def self_test():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # 言い換えた写し（FlashEnglish の「原則（5つ）」で実際にすり抜けた形）
        if run(good + "\n## 原則（5つ）\n1. トークン外を使わない\n") != 1:
