...
> **この表は初版と再監査（2026-09-06）の時点の数です。いまは違います**（規約23）。
> そのあと `104`→`199`/`200`/`201`、`105`→`203`、`92`→`202`、`58`→`204`、束ねの解消 `205`〜`212`、
> `213`（`--no-verify`）、`82`→`214`（両端2台の判断）と行を分けたので、
> **2026-09-07 時点で 214 行 / `依存 = none` は 82** です。
> **数を書き直すより、いま測る回し方を置きます**——次の1行で現在の数が出ます。
>
>     PYTHONUTF8=1 py build/inventory_check.py analysis/build-android-2026-09-06.json
...
### 段2 の突き合わせで名指しされた分の直し（2026-09-06・6件）

Mac mini の突き合わせ（`analysis/merge-build-2026-09-06.md`）で、**`依存` の列だけでは
仕分けきれなかった**と分かりました。同じ判断が片方で `none`・こちらで `env` や `hardware` に
置かれており、原因は**実体（コマンド）に引かれて分類していた**ことです。名指し分を直しました。

| id | 何を直したか |
|---|---|
| 21 | `flutter pub get`: **`env` → `none`。** 理由に書いた「Flutter SDK とネットワークに縛られる」は**どの機体でも同じ**で、`env` の理由になっていませんでした |
| 119 | 画面の大きさと密度の実測: **`hardware` → `none`。** 判断は「宣言値を信じず実測する」で対称。実体（`wm size` / `wm density`）だけが Android 固有 |
| 121 | はみ出しの警告の計数: **`hardware` → `none`。** 判断は「ログが綺麗なことを正常の根拠にしない」で対称。実体（logcat）だけが Android 固有 |
| 131 | **`工程` を `実機の確認` → `仮想機の確認`。** 実体が仮想機の道具の出力（`emulator_check.py:27-30, 412`）でした。規約自体は実機にも効きます |
| 103 | **2026-09-05 の決定が届いていませんでした。**「main への直 push を止めているのは Mac だけ」と書いていましたが、**この禁止は 2026-09-05 に廃止**されています |
| 104 | **5つの主張を1行に束ねていた**ので id 199・200・201 に分けました（iOS 210/213/214/223/225 と対応が付くように） |
| 82 | （2026-09-07 追記）**判断と機体のコマンドを束ねていた**ので id 214 に分けました。Mac mini が iOS `217`（最小と最大の2台で見る）を「Android の書き落とし」として名指ししましたが、実物（`aub origin/main:scripts/emulator_check.py:14-20, 73`）に判断は**あります**。**書き落としではなく粒度でした** |

これで **198 → 201 行**、`none` は **64 → 70**、`hardware` 18 → 16、`env` 62 → 61 です
（**この3つの数も 2026-09-06 時点**。上の測り方で現在の数を出してください）。

**103 の訂正で1つ分かったこと**があります。決定は廃止されたのに、
**`.githooks/pre-push:15-23` と design-harness の `ci/app-pre-push:19-26` には
`uname=Darwin` の判定が残っています**（この機体では hook が未接続なので実害は出ていません）。
コードが決定に追いついていない箇所として受信箱に上げました。
