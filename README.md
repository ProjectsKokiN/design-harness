...
| **`tools/impl_value_check.py`** | **実装の中だけの数値**を見つける（出どころが無い値）。`--components` は**部品が Figma 由来の数を持っていないか**を見る（名前付き定数でも逃げられない）。**`clamp` の係数はトークンから導く**（#104・2026-09-10。`N = トークン ÷ canvasWidth × 100`。版面幅は道具に埋め込まず案件の設定。上限が生値なら結びが切れるので名指し） |
...
| `tools/handoff_check.py` | 引き継ぎの記録（`SESSION_LOG.md`）が、いまの作業から離れていないか（#114）。先頭のエントリに**宣言した節**があるか・**数は時点のものだと断る一文**があるか・**最後にログを直してからコミットを重ねすぎていないか**の3つを見る。**中身は読まない**<br>書式は案件が `design/handoff.json` に宣言する。それまでこの検査はハーネスに無く、**案件ごとに実装がバラバラ**だった（aub は Python・FlashEnglish は Dart・PlantTalk は無し）。aub の実害: 先頭のエントリが「12段・149件」のまま**64 コミット**入っていなかった（実際は 26 段・604 件） |
...
## 機体で結果が変わる書き方（機械で見ていないもの）

`portable_check` が**見ている**のは、`str(Path)` の比較・`.replace("/", …)`・
`str(Path.relative_to(...))`・`encoding=` の欠落・外部コマンドの名前起動です。

**次のものは見ていません。決まりとして守ってください。**
...
**溢れる関門はまとめて宣言されて意味を失う**ので、入れていません
（Windows の薦め・確信度 中を、この実測を理由に採りませんでした）。
